...
\paragraph{Parallel} Many recent works on fast quadtree construction using the GPU are limited either to point sites \cite{bedorf2012sparse,karras2012maximizing,zhou2011data} \blue{or to sites that don't overlap octree cells \cite{li2014fully}.}
...
\blue{Most quadtree approaches that support surfaces are designed for efficient rendering and not inter-object resolution. Most of these approaches construct the quadtree on the CPU \cite{baert2013out,crassin2009gigavoxels,laine2011efficient,lefebvre2007compressed}, although Choi \etal \cite{choi2010parallel} succeed in constructing k-D trees in parallel.} Two works \cite{bastos2008gpu,park2010cuda} implement Adaptive Distance Fields in parallel on quadtrees but building the quadtree itself is done sequentially.  Yin \etal~\shortcite{yin2011fast} \blue{and Crassin and Green \cite{crassin2012octree}} compute the octree entirely on the GPU using a bottom-up approach by initially subdividing into a complete octree, resulting in memory usage that is no better than using a uniform grid.
...
We refer to quadtree leaf cells that intersect two or more objects as ``conflict cells.'' A necessary and sufficient condition for a quadtree to resolve objects is to have no conflict cells. Our approach to computing such a quadtree is \blue{in two stages. We first build an initial quadtree, called the ``vertex quadtree,'' using a set $S$ of point samples. We initialize $S$ to be the object vertices. The second stage is to detect conflict cells in parallel, followed by augmenting $S$ with sample points such that a subsequent quadtree built on $S$ resolves conflict cells.} If $S$ changed, then we iterate (see section \ref{sec:iterate}) \blue{which is necessary only if a conflict cell has multiple intersecting objects and is minimized by starting from an initial vertex quadtree}. \blue{This two-stage approach enabled by our sample point computation is the first method that can resolve between objects fully in parallel regardless of object spacing, i.e., we do not iterate through levels of the quadtree, subdividing as we go.}
...
We then propagate the BRT labels up the tree in parallel, marking ``unknown'' nodes as ``required'' when the labels of the current node's two child nodes don't match. \blue{Labels are applied using an atomic compare and swap, and threads terminate if the current ancestor's label was previously ``unknown''.} Finally, we generate quadtree nodes from only the required internal binary radix tree nodes.
...
\blue{
\subsection{Identifying conflict cells}
After the pruning in \ref{sec:pruning}, the Karras quadtree separates all differently labeled vertices in the dataset. Our goal is to separate differently labeled facets. We first need to identify what quadtree cells require further subdivision. We call these cells ``conflict cells'' (see figure \ref{fig:conflict-find-5}). To efficiently identify conflict cells, we take advantage of the space filling Morton curve and the existing quadtree hierarchy to reduce the combinatoric complexity of intersection detection between facets and quadtree cells. We use the following approach.}

\blue{\subsubsection{Initializing conflict cell detection}
\label{sec:initconflict}
Before we detect conflict cells we create a mapping from each quadtree cell $c$ to all facets bounded by $c$, a technique similar in spirit to the fragment emission and sorting done by Pantaleoni \cite{pantaleoni2011voxelpipe}. We first find the ``bounding cell'' $c_f$ for a facet $f$, where the bounding cell is the smallest quadtree cell that completely contains $f$. For each facet $f$ in parallel we determine $lcp_f$, which is the longest common prefix of the Morton codes of the vertices of $f$. Then, to find the bounding cell $c_f$, we iterate in parallel over each facet $f$ and use $lcp_f$ to direct a search through the quadtree. Let $F$ be the number of facets. We allocate two parallel arrays of size $F$, \texttt{BCells} and \texttt{FacetMap}. As each $c_f$ is found, the index of $c_f$ is stored in the \texttt{BCells} array at index equal to the thread id. At the same time, we store the index of each facet in the \texttt{FacetMap}. Initially, {FacetMap[i] = i} (see figure \ref{fig:data-structures-1}).}

\blue{
Next, we perform a parallel radix sort on the parallel arrays (\texttt{BCells} and \texttt{FacetMap}) using the bounding cell addresses as the sort key (figure \ref{fig:data-structures-2}). Finally, since each quadtree cell may bound multiple facets, we compute a range for each quadtree cell by comparing neighboring quadtree indices in the mapping in parallel (the \texttt{(F/L)Facet} array in figure \ref{fig:data-structures-2}). We now have a mapping from a quadtree cell $c$ to facets bounded by $c$ (figure \ref{fig:data-structures-3}).}
...
\begin{figure}
  ...
  \subfloat[][]{
    ...
    \includegraphics[width=0.3\columnwidth]{conflict-resolution-x.pdf} }
  \subfloat[][]{
    \label{fig:conflict-resolution-even}
    \includegraphics[width=0.3\columnwidth]{conflict-resolution-even.pdf} }
  \subfloat[][]{
    \label{fig:conflict-resolution-all}
    \includegraphics[width=0.3\columnwidth]{conflict-resolution-all.pdf} }\\
  \subfloat[][]{
    \label{fig:conflict-resolution-quadtree}
    \includegraphics[width=0.3\columnwidth]{conflict-resolution-octree.pdf} }
  \subfloat[][]{
    \label{fig:conflict-resolution-adjacent-even}
    \includegraphics[width=0.3\columnwidth]{conflict-resolution-adjacent-even.pdf} }
  \subfloat[][]{
    \label{fig:conflict-resolution-adjacent-all}
    \includegraphics[width=0.3\columnwidth]{conflict-resolution-adjacent-all.pdf} }
  \caption{
    \protect\subref{fig:conflict-resolution-x} A conflict cell with two lines from different objects.
    \protect\subref{fig:conflict-resolution-even}-\protect\subref{fig:conflict-resolution-all} Fitting boxes such that any box intersecting both lines contains at least one sample (red dots).
    ...
    \blue{\protect\subref{fig:conflict-resolution-adjacent-even}-\protect\subref{fig:conflict-resolution-adjacent-all} The adjacent case.}
  }
...
We consider only cases where the slope of $p$ is in the range $0 \le m \le 1$. All other instances can be transformed to this case using rotation and reflection. We begin by fitting the smallest box centered on a point $p$ that intersects both $q$ and $r$. \blue{The smallest box sampled at point $p(s)$ has edge length $a(s)$ as shown in figure \ref{fig:conflict-resolution-even}. We break the problem of finding $a(s)$ into two cases:}
...
\blue{We have implemented\footnote{
Source code will be made available at our website. %is available at \url{http://cedmav.org/research/project/33-gvds.html}.
}
our algorithm using OpenCL. Figure \ref{fig:flow-chart} shows the stages of the major kernels. Every kernel call is parallel on vertices, facets, quadtree nodes or Morton code bits. Our implementation uses 64-bit Morton codes, which were sufficient for all of our datasets. There is no way of knowing \textit{a priori} what the object spacing is going to be, however, 64-bit codes were sufficient for the most demanding datasets while retaining reasonable radix-sort timings.
Choosing a large number of bits for the Morton code results in little or no wasted effort in later refinement. Increasing the size of the Morton codes affects the amount of later pruning only if it results in extraneous resolution of vertices in the initial vertex quadtree computation. The number of bits also has no effect on the number of conflict cells unless the Morton codes are too small to resolve vertices.}

\blue{Our implementation of the algorithm supports polygons and polylines which needn't be manifold or connected. Intersecting lines are not handled as a special case, i.e., the quadtree is simply resolved to its maximum depth.}

\begin{figure}
  \centering
  % trim={<left> <lower> <right> <upper>}
  \includegraphics[trim={0 50mm 0 0},clip,width=0.95\columnwidth]{kernel-stages.pdf}
  \caption{Kernel calls, with some calls omitted for clarity. The name of the kernel is in larger font while the the elements on which the parallelism runs are given in smaller font. The majority of calls are facet- or vertex-parallel.
  }
...
\begin{figure}
  \centering
%  \subfloat[][]{
%    \label{fig:timings-data}
%    % trim={<left> <lower> <right> <upper>}
%    \includegraphics[width=0.3\columnwidth]{facet-test-image} }\\
  \subfloat[][]{
    \label{fig:timings1}
    % trim={<left> <lower> <right> <upper>}
    \includegraphics[trim={4mm 0 40mm 0},clip,width=0.48\columnwidth]{facetxtime} }
  \subfloat[][]{
    \label{fig:timings2}
    \includegraphics[trim={4mm 0 40mm 0},clip,width=0.48\columnwidth]{facetxcells.png} }
  \caption{Scaling tests. The dataset used is a square domain with increasing numbers of non-intersecting lines.
%    \protect\subref{fig:timings-data} Data used in the scaling study. Lines are shortened and added to the perimeter of the square to increase the number of facets.
    \protect\subref{fig:timings1} Our algorithm consistently performs an order of magnitude faster than the algorithm of Edwards \etal \cite{edwards2015approximating} as the number of facets increases.
    \protect\subref{fig:timings2} The quadtree that we build is roughly equal in size to that of the previous work.
  }
  \label{fig:timings}
\end{figure}
