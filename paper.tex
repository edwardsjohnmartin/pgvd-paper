...
Choosing a large number of bits for the Morton code results in little or no wasted effort in later refinement, affecting the amount of later pruning only if intra-object vertex spacing is small compared to spacing between objects. The number of bits also has no effect on the number of conflict cells unless the Morton codes are too small to resolve vertices.}

\blue{Our implementation of the algorithm supports polygons and polylines which needn't be manifold or connected. Intersecting lines are not handled as a special case, i.e., the quadtree is simply resolved to its maximum depth. Special handling can be implemented per application as needed, e.g., for collision detection applications.}
