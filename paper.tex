...
\subsection{Detecting conflict cells}

Let the ``quadtree address'' refer to the unique ID of a quadtree cell $C$ found by concatenating the local addresses of its ancestors from Root to $C$. The address of the root cell is a special case and is defined as $R$. Figure \ref{fig:octree-cartesian-5} shows the addresses of each leaf cell in the quadtree.

We define a smallest containing cell (SCC) to be the smallest internal quadtree node which entirely contains a given facet. Given a facet defined by two endpoints $p$ and $q$, the quadtree address of the SCC is the longest common prefix of the Morton codes of $p$ and $q$. In figure \ref{fig:octree-cartesian-3}, the address of the SCC of facet D is 10. The SCC of facet E is $R$.
...
\begin{figure}
  ...
  \begin{minipage}{0.3\columnwidth}
    ...
    0110100
  \end{minipage}
  \caption{Sorting the SCC-facet pairs. We first sort lexicographically (\textit{lexsort}), then by length (\textit{lensort}), using a parallel radix sort. In the \textit{lexsort}, we implicitly pad with zeros on the right so that all values are the same length. \red{How exactly is the \textit{lensort} done?}}
  \label{fig:scc-sort}
\end{figure}
...
\begin{figure}
  ...
  \subfloat[][]{
    ...
    \includegraphics[width=0.48\columnwidth]{conflict-resolution-octree.pdf} } \\
  \subfloat[][]{
    \label{fig:conflict-resolution-adjacent-even}
    \includegraphics[width=0.48\columnwidth]{conflict-resolution-adjacent-even.pdf} }
  \subfloat[][]{
    \label{fig:conflict-resolution-adjacent-all}
    \includegraphics[width=0.48\columnwidth]{conflict-resolution-adjacent-all.pdf} }
  \caption{
